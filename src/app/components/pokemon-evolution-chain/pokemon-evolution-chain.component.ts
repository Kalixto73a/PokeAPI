...
export class PokemonEvolutionChainComponent implements OnInit {
      ...
      next: (speciesData) => {
        ...
        if (evolutionUrl) {
          this.evolutionChainService
            .getPokemonEvolutionChain(evolutionUrl)
            .subscribe((chainData) => {
              this.evolutionChain = chainData
              const observables = this.getEvolutionChainOfPokemon(
                chainData.chain,
              ).map((p) => {
                const id = this.getPokemonIdFromSpeciesUrl(p.species.url)
                return this.detailsForEachPokemonService.getDetailsOfPokemon(
                  id,
                )
              })
  ...
  public getEvolutionChainOfPokemon(
    result: EvolutionChainOfPokemon,
  ): EvolutionChainOfPokemon[] {
  ...
  public loadPokemonTypes(pokemonId: number): void {
    ...
    this.detailsForEachPokemonService
      .getDetailsOfPokemon(pokemonId)
      .subscribe((pokemonData) => {
        this.pokemonTypesMap[pokemonId] = pokemonData.types
      })
  }

  public getValidEvolutionRequirements(
    ...
    switch (detail.trigger.name) {
      case 'level-up':
        if (detail.min_happiness && detail.time_of_day)
          return `Having a minimum happiness of  ${detail.min_happiness} at ${detail.time_of_day}`
        if (detail.min_happiness && detail.known_move_type)
          return `Having a minimum happiness of  ${detail.min_happiness} and knowing ${this.toTitleCase(detail.known_move_type.name)} type movement`
        if (detail.min_affection && detail.known_move_type)
          return `Having a minimum affection of ${detail.min_affection} hearts and knowing ${this.toTitleCase(detail.known_move_type.name)} type movement`
        if (detail.min_level && detail.party_type)
          return `Evolves at level ${detail.min_level} with a ${detail.party_type.name} type pokemon on the team`
        if (detail.min_level && detail.turn_upside_down)
          return `Evolves at level ${detail.min_level} and turning the console upside down`
        if (detail.min_level && detail.relative_physical_stats !== null)
          return `Evolves at level ${detail.min_level} and ${this.getPhysicalStatsText(detail.relative_physical_stats)}`
        if (detail.min_level && detail.time_of_day)
          return `Evolves at level ${detail.min_level} at ${detail.time_of_day}`
        if (detail.min_level && detail.needs_overworld_rain)
          return `Evolves at level ${detail.min_level} and while it rains`
        if (detail.min_beauty)
          return `Having minimum beauty of ${detail.min_beauty}`
        if (detail.min_steps)
          return `Walking a minimum of ${detail.min_steps} steps`
        if (detail.min_affection)
          return `Having a minimum affection of ${detail.min_affection} hearts`
        if (detail.min_happiness)
          return `Having a minimum happiness of  ${detail.min_happiness}`
        if (detail.party_species)
          return `Having ${this.toTitleCase(detail.party_species.name)} in the team`
        if (detail.location)
          return `Leveling up in ${this.toTitleCase(detail.location.name)}`
        if (detail.min_level) return `Evolves at level ${detail.min_level}`
        if (detail.min_steps) return `minumum steps walked`
        if (detail.known_move)
          return `Knowing ${this.toTitleCase(detail.known_move.name.replace('-', ' '))}`
        return `level up`
      case 'use-item':
        if (detail.item && detail.gender)
          return `Being ${this.getGenderTypeText(detail.gender)} and using ${this.toTitleCase(detail.item.name.replace('-', ' '))}`
        if (detail.item)
          return `Using ${this.toTitleCase(detail.item.name.replace('-', ' '))}`
        return 'Using an item'
      case 'trade':
        if (detail.trade_species)
          return `Trading for a ${this.toTitleCase(detail.trade_species.name)}`
        if (detail.held_item)
          return `Tading the pokemon holding ${this.toTitleCase(detail.held_item.name.replace('-', ' '))}`
        return `Trading`
      case 'use-move':
        if (detail.min_move_count)
          return `Using ${detail.used_move?.name} ${detail.min_move_count} times`
